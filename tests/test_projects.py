...
def test_create_with_invalid_data(client, auth):
    auth.login(1)
...
def test_create_with_valid_data(client, auth):
    auth.login(1)
    rsp = client.post(
        '/projects/create',
    ...
    assert user_projects.count() == 4
    assert (
        b'You can not create any more projects, you can only create or join 4 or less '
        b'projects.' in rsp.data
    )


def test_update_with_invalid_data(client, auth):
    auth.login(1)
    ...
    rsp = client.post(
        '/projects/1/update',
        data={'title': 'test_title_1', 'description': 'test_description_1'},
        follow_redirects=True,
    )
    assert b'No changes have been made.' in rsp.data
...
def test_update_with_valid_data(client, auth):
    ...
    assert 'http://localhost/projects/1/manage' == rsp.headers['Location']
